...
function createDbChain(finalResult: { data?: unknown; error?: unknown; count?: number | null }) {
...
describe('checkAutoApproval', () => {
  ...
  it('debe permitir riesgo 2 cuando no hay trust record (comportamiento por defecto)', async () => {
    ...
    const trustChain = createDbChain({ data: null, error: { code: 'PGRST116', message: 'Not found' } })
    const insertChain = createDbChain({ data: null, error: { message: 'Insert failed' } })
    let callCount = 0
  ...
  it('debe rechazar riesgo 3 sin trust record', async () => {
    const trustChain = createDbChain({ data: null, error: { code: 'PGRST116' } })
    const insertChain = createDbChain({ data: null, error: { message: 'fail' } })
    let callCount = 0
    mockClient.from.mockImplementation(() => {
      callCount++
      if (callCount === 1) return trustChain
...
describe('checkRateLimit', () => {
  ...
  it('debe rechazar cuando se excede límite por minuto', async () => {
    ...
    ;(auditChain.single as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 5, error: null })
    // But select with head: true returns count directly via the chain
    ;(auditChain.gte as ReturnType<typeof vi.fn>).mockReturnValue({ count: 5, error: null })
