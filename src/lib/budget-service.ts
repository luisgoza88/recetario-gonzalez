 ...
 */
export async function estimateShoppingCost(
...
export async function getSpendingByCategory(
  ...
  let query = supabase.from("purchases").select("item_id, item_name, price");
  ...
  for (const purchase of purchases) {
    const category =
      (purchase.item_id && categoryMap.get(purchase.item_id)) || "Otros";
...
export async function getBudgetAlerts(): Promise<string[]> {
  ...
  if (summary.isOverBudget) {
    alerts.push(
      `Has excedido tu presupuesto por $${Math.abs(Math.round(summary.remaining)).toLocaleString()}`,
    );
  } else if (summary.percentUsed > 80) {
    alerts.push(
      `Has usado el ${Math.round(summary.percentUsed)}% de tu presupuesto`,
    );
  }

  if (
    summary.projectedTotal > summary.budget.budget_amount * 1.1 &&
    ...
    alerts.push(
      `A este ritmo, excederás tu presupuesto por ~$${excess.toLocaleString()}`,
    );
  }
