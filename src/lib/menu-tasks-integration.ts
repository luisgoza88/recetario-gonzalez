...
function analyzeRecipeForTasks(
    ...
    for (const rule of PREP_RULES) {
      ...
      if (matches) {
        ...
        const taskDateTime = new Date(
          mealDateTime.getTime() - rule.minutesBefore * 60 * 1000,
        );
  ...
  if (recipe.total_time || recipe.cook_time) {
    ...
    const mealDateTime = new Date(`${scheduledDate}T${mealTime}:00`);
    const cookStartTime = new Date(
      mealDateTime.getTime() - cookTime * 60 * 1000,
    );
 ...
 */
export async function generateKitchenTasks(
  daysAhead: number = 3,
): Promise<KitchenTask[]> {
  ...
  // Obtener menú
  const { data: menus } = await supabase.from("day_menu").select(`
      *,
  ...
  for (let i = 0; i < daysAhead; i++) {
    ...
    const menu =
      menus.find((m) => m.day_number === dayIndex) || menus[i % menus.length];

    if (!menu) continue;

    // Analizar cada comida
    if (menu.breakfast) {
      const breakfastTasks = analyzeRecipeForTasks(
        menu.breakfast as Recipe,
        "breakfast",
        targetDateStr,
      );
    ...
    if (menu.lunch) {
      const lunchTasks = analyzeRecipeForTasks(
        menu.lunch as Recipe,
        "lunch",
        targetDateStr,
      );
    ...
    if (menu.dinner) {
      const dinnerTasks = analyzeRecipeForTasks(
        menu.dinner as Recipe,
        "dinner",
        targetDateStr,
      );
...
export async function getPrepSummary(): Promise<{
  ...
  const urgentTasks = tasks.filter((t) => {
    const taskTime = new Date(
      `${t.scheduled_date}T${t.scheduled_time || "00:00"}`,
    );
 ...
 */
export async function syncWithHouseholdTasks(
  householdId: string,
): Promise<void> {
