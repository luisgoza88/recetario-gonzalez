...
// Day Menus
export async function cacheDayMenus(
  menus: RecetarioDBSchema["cachedDayMenus"]["value"][],
): Promise<void> {
...
export async function getCachedDayMenus(): Promise<
  RecetarioDBSchema["cachedDayMenus"]["value"][]
> {
  const db = await getDB();
  return db.getAll("cachedDayMenus");
}

export async function getCachedDayMenu(
  dayNumber: number,
): Promise<RecetarioDBSchema["cachedDayMenus"]["value"] | undefined> {
  const db = await getDB();
...
export async function cacheRecipes(
  recipes: RecetarioDBSchema["cachedRecipes"]["value"][],
): Promise<void> {
  const db = await getDB();
...
export async function getCachedRecipes(): Promise<
  RecetarioDBSchema["cachedRecipes"]["value"][]
> {
  const db = await getDB();
...
export async function getCachedRecipe(
  id: string,
): Promise<RecetarioDBSchema["cachedRecipes"]["value"] | undefined> {
  const db = await getDB();
  return db.get("cachedRecipes", id);
}

// Market Items
export async function cacheMarketItems(
  items: RecetarioDBSchema["cachedMarketItems"]["value"][],
): Promise<void> {
  const db = await getDB();
...
export async function getCachedMarketItems(): Promise<
  RecetarioDBSchema["cachedMarketItems"]["value"][]
> {
  const db = await getDB();
...
export async function cacheInventory(
  items: RecetarioDBSchema["cachedInventory"]["value"][],
): Promise<void> {
  const db = await getDB();
...
export async function getCachedInventory(): Promise<
  RecetarioDBSchema["cachedInventory"]["value"][]
> {
  const db = await getDB();
...
export async function getCacheStats(): Promise<{
  ...
  const db = await getDB();
  const [dayMenus, recipes, marketItems, inventory, pendingOps] =
    await Promise.all([
...
export async function addFrequentItem(item: {
  id: string;
  name: string;
}): Promise<void> {
  const db = await getDB();
...
export async function getFrequentItems(
  limit = 10,
): Promise<RecetarioDBSchema["frequentItems"]["value"][]> {
  const db = await getDB();
