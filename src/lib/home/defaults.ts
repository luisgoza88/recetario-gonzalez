 ...
 */
export function getDefaultTaskDuration(
  taskName: string,
  spaceType?: string,
): number {
  ...
  if (spaceType) {
    ...
    for (const [spaceKey, duration] of Object.entries(
      DEFAULT_DURATION_BY_SPACE_TYPE,
    )) {
      if (
        normalizedSpace.includes(spaceKey) ||
        spaceKey.includes(normalizedSpace)
      ) {
        return duration;
      }
    }
...
export function calculateBalanceTargets(
  ...
  const maxPerEmployee = Math.ceil(
    targetPerEmployee * (1 + bufferPercent / 100),
  );
...
export function calculateWeeklyMinutesForSpace(spaceType: string): number {
  ...
  // Check interior
  for (const [type, tasks] of Object.entries(
    STANDARD_TASK_TEMPLATES.interior,
  )) {
  ...
  // Check exterior
  for (const [type, tasks] of Object.entries(
    STANDARD_TASK_TEMPLATES.exterior,
  )) {
    if (normalizedType.includes(type)) {
      return tasks.reduce((total, task) => {
        const frequencyDays = FREQUENCY_TO_DAYS[task.frequency] || 7;
