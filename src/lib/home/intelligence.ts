 ...
 */
export async function getSpaceTaskDurations(
  spaceId: string,
...
export async function calculateEmployeeScore(
  ...
  const tasksWithActualTime = completedTasks.filter(
    (t) => t.actual_minutes !== null,
  );
  ...
  const overallScore = Math.round(
    (avgRating / 5) * 25 + // 25% weight for ratings
      ...
      reliabilityScore * 0.3 + // 30% weight for reliability
  ...
  if (scheduledTasks) {
    for (const st of scheduledTasks) {
      ...
      if (taskData) {
        ...
        taskDurations[taskData.id] =
          learned?.learnedMinutes || taskData.estimated_minutes || 30;
      }
    }
  }
...
export async function suggestOptimalAssignment(
  ...
  Array<{ taskId: string; suggestedEmployeeId: string; reason: string }>
> {
  ...
  workloads.forEach((w) => {
    currentMinutes[w.employeeId] = w.assignedMinutes;
  });
    ...
    const employeeScores = eligibleEmployees.map((emp) => {
      ...
      const isPrimary = spaceAssignments?.some(
        (a) =>
          ...
          a.space_id === task.spaceId &&
          a.is_primary,
      );

      // Get priority order (lower = better)
      const priorityOrder =
        spaceAssignments?.find(
          (a) =>
            a.employee_id === emp.employeeId && a.space_id === task.spaceId,
        )?.priority_order || 999;
  ...
  for (let i = 0; i < daysAhead; i++) {
    ...
    if (workloads.length >= 2) {
      ...
      if (maxUtil - minUtil > 40) {
        const overworked = workloads.find(
          (w) => w.utilizationPercent === maxUtil,
        );
        const underworked = workloads.find(
          (w) => w.utilizationPercent === minUtil,
        );
...
export async function getIntelligenceSummary(householdId: string): Promise<{
  ...
  if (spaces) {
    for (const space of spaces) {
      ...
      durations.forEach((d) => {
        totalConfidence +=
          d.confidence === "high" ? 1 : d.confidence === "medium" ? 0.5 : 0.2;
      });
    }
  }
  ...
  return {
    totalLearnedTasks,
    avgConfidence:
      totalLearnedTasks > 0 ? totalConfidence / totalLearnedTasks : 0,
    employeeScores,
    weeklyPredictions,
  };
}
