...
export const dailyDashboardKeys = {
  tasks: (householdId: string, date: string) => ['dailyTasks', householdId, date] as const,
  checkins: (householdId: string, date: string) => ['dailyCheckins', householdId, date] as const,
};
...
async function fetchDailyCheckins(householdId: string, date: string) {
  ...
  if (data) {
    data.forEach((c: { employee_id: string; check_out_time: string | null; check_in_time: string }) => {
...
export function useToggleDailyTask() {
  ...
  return useMutation({
    mutationFn: async ({ task, householdId }: { task: ScheduledTask; householdId: string }) => {
      ...
      if (newStatus === 'completada') {
        const { error: historyError } = await supabase.from('cleaning_history').insert({
