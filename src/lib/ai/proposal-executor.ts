...
async function captureState(action: AIProposedAction): Promise<StateCapture[]> {
  ...
  try {
    ...
    switch (function_name) {
      ...
      case "delete_recipe": {
        const id =
          ...
          (parameters.recipe_id as string) ||
          (parameters.space_id as string) ||
          (parameters.employee_id as string);
    ...
    logger.error(`Error capturing state for ${function_name}`, {
...
export async function executeProposal(
      ...
      failed_actions: [
        {
          action_id: "unknown",
          function_name: "unknown",
          error: `Invalid status: ${proposal.status}`,
        },
      ],
      can_rollback: false,
    };
  ...
  const actionsToExecute =
    proposal.status === "partially_approved"
      ? proposal.actions.filter((a) =>
          proposal.approved_actions?.includes(a.id),
        )
      : proposal.actions;
  ...
  for (const action of actionsToExecute) {
    try {
      ...
      logger.error(`Error executing action ${action.function_name}`, {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
...
export async function executeActionWithLogging(
  ...
  try {
    ...
    // 2. Crear audit log
    const config = await import("./ai-command-service").then((m) =>
      m.getFunctionConfig(functionName),
    );
...
export async function rollbackProposal(
  ...
  if (
    ...
    proposal.audit_log_ids.length === 0
  ) {
    return {
      success: false,
      rolled_back: 0,
