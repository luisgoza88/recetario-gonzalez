...
function ingredientMatchesInventory(
  ...
  for (const sub of subIngredients) {
    ...
    for (const item of inventoryItems) {
      ...
      // Paso 1: Exacto
      if (normItem === sub) { found = true; break; }

      // Paso 2: Parcial (uno contiene al otro)
      if (normItem.includes(sub) || sub.includes(normItem)) { found = true; break; }
      ...
      if (subWords.some(sw => itemWords.some(iw => sw === iw || iw.includes(sw) || sw.includes(iw)))) {
        found = true;
        break;
      }
    }
...
export function countIngredientMatches(
  ...
  if (totalCount === 0) return { matchCount: 0, totalCount: 0, matchPercent: 0 };
...
export async function getTodayMenu() {
  try {
    ...
    if (error || !menu) {
      logger.error('Error fetching menu', { error: error instanceof Error ? error.message : String(error) });
      return {
    ...
    logger.error('getTodayMenu error', { error: err instanceof Error ? err.message : String(err) });
    return {
...
export async function getWeekMenu() {
  try {
    ...
    if (error) {
      logger.error('Error fetching week menu', { error: error instanceof Error ? error.message : String(error) });
    ...
    logger.error('getWeekMenu error', { error: err instanceof Error ? err.message : String(err) });
...
export async function searchRecipes(query: string) {
  try {
    ...
    if (error) {
      logger.error('Error searching recipes', { error: error instanceof Error ? error.message : String(error) });
    ...
    if (!recipes || recipes.length === 0) {
      ...
      if (allError) {
        logger.error('Error fetching all recipes', { error: allError instanceof Error ? allError.message : String(allError) });
        return { error: 'Error al buscar recetas' };
      }
    ...
    logger.error('searchRecipes error', { error: err instanceof Error ? err.message : String(err) });
    return { error: 'Error al buscar recetas' };
  }
}
...
export async function getInventory() {
  try {
    ...
    logger.error('getInventory error', { error: err instanceof Error ? err.message : String(err) });
    return { message: 'No se pudo obtener el inventario', items: [], by_category: {} };
  }
}
...
export async function getShoppingList() {
  try {
    ...
    if (error) {
      logger.error('Error fetching shopping list', { error: error instanceof Error ? error.message : String(error) });
    ...
    logger.error('getShoppingList error', { error: err instanceof Error ? err.message : String(err) });
    return { message: 'No hay items en la lista de compras', items: [] };
  }
}
...
export async function suggestRecipe(_preferences?: string) {
  ...
  const availableItems = (inventory?.map((i: Record<string, unknown>) => (i.market_item as { name?: string } | null)?.name).filter((n): n is string => !!n)) || [];
  ...
  for (const recipe of recipes) {
    const ingredients = Array.isArray(recipe.ingredients) ? recipe.ingredients : [];
    const { matchCount, matchPercent } = countIngredientMatches(ingredients, availableItems);
