...
import {
  getTodayMenu, getWeekMenu, getRecipeDetails, getMissingIngredients,
  getInventory, getShoppingList, suggestRecipe, countIngredientMatches
...
import { getCurrentDateInfo, getLowInventoryAlerts, getPreparationTips, smartShoppingList, getUpcomingMeals } from './reports';
...
export async function executeMultiStepTask(
  ...
  switch (taskType) {
    case 'prepare_recipe': {
      ...
      if (!recipeName) {
        return {
          task_type: taskType, steps_completed: [],
      ...
      if ('recipe_not_found' in recipeDetails && recipeDetails.recipe_not_found) {
        ...
        return {
          task_type: taskType, steps_completed: stepsCompleted, results,
    ...
    case 'menu_from_inventory': {
      ...
      const availableItems = invItems?.map(i => (i.market_item as { name?: string })?.name).filter((n): n is string => !!n) || [];

      for (const recipe of recipes || []) {
        const ingredients = Array.isArray(recipe.ingredients) ? recipe.ingredients : [];
        const { matchPercent } = countIngredientMatches(ingredients, availableItems);
