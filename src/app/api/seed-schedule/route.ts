...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const emp of seedEmployees) {
      ...
      if (error || !created?.id) {
        logger.error(`Error creating employee ${emp.name}`, {
    ...
    if (replaceExisting) {
      ...
      if (seededIds.length > 0) {
        ...
        await supabase.from("task_templates").delete().in("id", seededIds);
      }
    }
    ...
    for (const emp of seedEmployees) {
      ...
      for (const g of grouped) {
        ...
        const avgDuration = Math.round(
          g.durations.reduce((sum, d) => sum + d, 0) /
            Math.max(1, g.durations.length),
        );

        const firstDescription = g.descriptions[0]
          ? ` | ${g.descriptions[0]}`
          : "";
    ...
    if (templatesToInsert.length > 0) {
      ...
      if (insertError) {
        logger.error("Error inserting seed templates", {
          error:
    ...
    logger.error("Seed schedule error", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: templates, count } = await supabase
      .from("task_templates")
      .select(
        "id, frequency, assigned_employee_id, employee:home_employees(id, name)",
        { count: "exact" },
      )
      .eq("household_id", householdId)
      .ilike("description", `${SEED_TAG}%`);
    ...
    return NextResponse.json({
      ...
      byFrequency,
      employees:
        employees?.map((e) => ({ id: e.id, name: e.name, zone: e.zone })) || [],
    });
  } catch (error) {
    logger.error("Get schedule stats error", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      { error: "Internal server error" },
