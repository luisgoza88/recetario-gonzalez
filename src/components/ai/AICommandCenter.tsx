...
function RiskBadge({ level }: { level: number }) {
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${RISK_COLORS[level]}`}
    >
      {RISK_LABELS[level]}
    </span>
  );
...
function ProposalCard({
  ...
  const expiresIn = Math.max(
    0,
    Math.floor((new Date(proposal.expires_at).getTime() - Date.now()) / 60000),
  );
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-gray-400">
              {proposal.actions.length} acción
...
export default function AICommandCenter({
  ...
  // Nota: El chat se eliminó de aquí porque es redundante con FloatingAIAssistant
  // Este componente ahora solo maneja: Dashboard, Propuestas, Historial y Configuración
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "proposals" | "history" | "settings"
  >("dashboard");
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl p-4 text-white">
                ...
                <p className="text-xs text-purple-200 mt-2">
                  Auto-aprueba acciones de riesgo{" "}
                  {RISK_LABELS[trustStats?.auto_approve_level || 1]} o menor
                </p>
              </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-yellow-800">
                      {pendingProposals.length} propuesta
                      {pendingProposals.length !== 1 ? "s" : ""} pendiente
              ...
              {pendingProposals.length === 0 ? (
                ...
                pendingProposals.map((proposal) => (
                  <ProposalCard
                    ...
                    proposal={proposal}
                    onApprove={() =>
                      handleApproveProposal(proposal.proposal_id)
                    }
          ...
          {activeTab === "history" && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100">
                ...
                <p className="text-sm text-gray-500">
                  Últimas 20 acciones ejecutadas
                </p>
              </div>
              <div className="px-4">
                {auditLog.map((entry) => (
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="font-medium text-gray-800">
                          Nivel de Auto-Aprobación
                        </p>
                        <p className="text-sm text-gray-500">
                          Acciones con este riesgo o menor se ejecutan
                          automáticamente
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                      {[1, 2, 3, 4].map((level) => (
                        <button
                          key={level}
                          onClick={() =>
                            handleUpdateTrust("auto_approve_level", level)
                          }
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-sm">
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="font-medium">
                            {trustStats.max_actions_per_minute}
                          </span>
                        </div>
                        <input
                          type="range"
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm text-gray-600">
                            Acciones críticas por día
                          </span>
                          <span className="font-medium">
                            {trustStats.max_critical_actions_per_day}
                          </span>
                        </div>
                        <input
                          type="range"
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm text-gray-600">
                            Items máx. por operación masiva
                          </span>
                          <span className="font-medium">
                            {trustStats.max_items_per_bulk_operation}
                          </span>
                        </div>
                        <input
                          type="range"
                    ...
                    {[
                      {
                        ...
                        description:
                          "Todas las acciones requieren aprobación manual",
                        icon: <Lock size={20} />,
                      ...
                      },
                      {
                        ...
                        description:
                          "La IA puede modificar múltiples registros a la vez",
                      ...
                      },
                      {
                        ...
                        description:
                          "La IA puede eliminar datos permanentemente",
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleUpdateTrust(setting.field, !setting.value)
                          }
                    ...
                    <div className="grid grid-cols-4 gap-2 text-center">
                      {[1, 2, 3, 4].map((level) => {
                        const count = functions.filter(
                          (f) => f.risk_level === level,
                        ).length;
                        return (
                          <div
                            key={level}
                            className={`py-2 px-1 rounded-lg ${RISK_COLORS[level]}`}
                          >
