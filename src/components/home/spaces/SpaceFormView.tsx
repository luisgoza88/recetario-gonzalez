...
import {
  Check, Trash2, Clock, ChevronDown, ChevronUp, Plus, Scan
} from 'lucide-react';
...
interface SpaceFormViewProps {
  ...
  onUpdateAttribute: (attr: keyof SpaceAttributes, value: boolean | number | string) => void;
  onToggleTask: (index: number) => void;
  onUpdateTaskFrequency: (index: number, frequency: TaskConfig['frequency']) => void;
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => onUpdateSpace({ ...editingSpace, customName: e.target.value })}
      ...
      {editingSpace.category === 'interior' && !selectedType?.name?.toLowerCase().includes('baño') && (
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-3">
            ...
            <div className="flex items-center gap-3 bg-white rounded-lg p-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => onUpdateAttribute('has_windows', Math.max(0, editingSpace.attributes.has_windows - 1))}
                ...
                <button
                  onClick={() => onUpdateAttribute('has_windows', editingSpace.attributes.has_windows + 1)}
                  className="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 font-bold hover:bg-gray-200"
                >
                  +
                </button>
            ...
            <div className="bg-white rounded-lg p-2">
              ...
              <div className="flex flex-wrap gap-1">
                {[
                  ...
                  <button
                    key={floor.value}
                    onClick={() => onUpdateAttribute('floor_type', floor.value)}
            ...
            <div className="flex items-center gap-3 bg-white rounded-lg p-2">
              ...
              <input
                ...
                onChange={(e) => onUpdateSpace({ ...editingSpace, areaSqm: parseFloat(e.target.value) || 0 })}
                placeholder="Ej: 25"
                  ...
                  {task.enabled && (
                    <div className="flex gap-1 mt-2 ml-7">
                      {FREQUENCY_OPTIONS.map(freq => (
                        <button
                          key={freq.value}
                          onClick={() => onUpdateTaskFrequency(index, freq.value as TaskConfig['frequency'])}
                          className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
