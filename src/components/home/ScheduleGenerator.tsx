...
const FREQUENCY_BY_USAGE = {
  alto: {
    daily: ["Lavar platos", "Tender cama", "Sacar basura", "Regar plantas"],
    weekly: 2,
    biweekly: 0,
  },
...
export default function ScheduleGenerator({
  ...
  const getFrequencyDays = (task: string, usageLevel: string): number => {
    const usage =
      FREQUENCY_BY_USAGE[usageLevel as keyof typeof FREQUENCY_BY_USAGE];
    ...
    if (useIntelligence && availableEmployees.length > 1) {
      ...
      const scored = availableEmployees.map((emp) => {
        const currentMinutes =
          minutesAssignments.get(`${emp.id}-${dateKey}`) || 0;
      ...
      let minMinutes =
        minutesAssignments.get(`${bestEmployee.id}-${dateKey}`) || 0;
  ...
  const generateSchedule = async () => {
    ...
    for (const space of spaces) {
      ...
      for (const taskName of tasks) {
          ...
          if (existingTask) {
            try {
              ...
              estimatedMinutes =
                ...
                getDefaultTaskDuration(taskName, spaceTypeName);
            }
          } else {
        ...
        currentDate.setDate(
          currentDate.getDate() + (spaceIndex % frequencyDays),
        );
        ...
        while (currentDate < endDate) {
          ...
          if (employee) {
            ...
            minutesAssignments.set(
              empKey,
              (minutesAssignments.get(empKey) || 0) + estimatedMinutes,
            );
          }
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      const uniqueNewTemplates = new Map<
        string,
        {
          household_id: string;
          space_id: string;
      ...
      for (const task of preview) {
        ...
        if (
          ...
          !uniqueNewTemplates.has(templateKey)
        ) {
      ...
      for (let i = 0; i < tasksToInsert.length; i += batchSize) {
        ...
        const { error } = await supabase.from("scheduled_tasks").insert(batch);
  ...
  const tasksByDate = preview.reduce(
    (acc, task) => {
  ...
  return (
    <FocusTrap active={true}>
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="schedule-generator-title"
        ...
        <div className="bg-white rounded-2xl w-full max-w-lg">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white p-4 rounded-t-2xl flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Sparkles size={20} />
              <span id="schedule-generator-title" className="font-semibold">
              ...
              <>
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Generaremos un calendario de limpieza basado en tus espacios
                    y empleados
                  </p>
                </div>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4">
                  ...
                  {useIntelligence ? (
                    ...
                    <p className="text-sm text-gray-600">
                      Usa tiempos estimados y balance simple por número de
                      tareas
                    </p>
                  )}
                </div>
              ...
              <>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">
                    {preview.length} tareas en {Object.keys(tasksByDate).length}{" "}
                    días
                  </span>
                </div>
