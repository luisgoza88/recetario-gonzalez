...
export default function ScheduleDashboard({
  ...
  const loadDailyTasks = useCallback(async () => {
    ...
    const [{ count: templateCount }, { data: tasksData }] = await Promise.all([
      ...
      supabase
        ...
        .select(
          "id, employee_id, status, started_at, completed_at, notes, task_template:task_templates(id, name, category, estimated_minutes, priority)",
        )
        .eq("household_id", householdId)
    ...
    const progress = cards
      .map((card) => {
        const tasksList = tasks.filter((task) =>
          card.id === "__unassigned__"
            ? !task.employee_id
            : task.employee_id === card.id,
        );
        ...
        const pending = tasksList.filter(
          (t) => t.status === "pendiente" || t.status === "omitida",
        ).length;
  ...
  const generateDailyTasks = async () => {
    ...
    try {
      ...
      const activeTemplates = (templates ||
        []) as unknown as TaskTemplateSeed[];
      const applicableTemplates = activeTemplates.filter((template) =>
        shouldScheduleTemplate(template, targetDate),
      );
  ...
  const updateTaskStatus = async (
    ...
    await supabase.from("scheduled_tasks").update(updates).eq("id", taskId);
  ...
  const totalCompleted = employeeProgress.reduce(
    (sum, ep) => sum + ep.completed,
    0,
  );
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              <Spinner size="xl" color="indigo" className="mb-4" />
            ...
            <>
              ...
              {employeeProgress.map((ep) => (
                <div
                  key={ep.employee.id}
                  className="bg-white border rounded-xl mb-3 overflow-hidden"
                >
                  <button
                    onClick={() =>
                      setExpandedEmployee(
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        ep.employee.zone === "interior"
                          ? "bg-blue-100"
                          : ep.employee.zone === "exterior"
                            ? "bg-green-100"
                            : "bg-purple-100"
                      }`}
                    >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <div className="text-sm text-gray-500">
                        {ep.completed}/{ep.total} completadas (
                        {ep.progressPercent}%)
                      </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                <span
                                  className={`text-xs px-2 py-0.5 rounded-full ${category.bg} ${category.text}`}
                                >
                                  {category.icon}{" "}
                                  {template?.category || "general"}
                                </span>
