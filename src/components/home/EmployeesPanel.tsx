...
import { X, Plus, User, Trash2, Edit2, Check, Clock } from "lucide-react";
...
const legacyToSchedule = (
  workDays: string[],
  hoursPerDay: number,
  existingSchedule?: Record<string, DaySchedule>,
...
export default function EmployeesPanel({
  ...
  const [editingEmployee, setEditingEmployee] = useState<EmployeeForm | null>(
    null,
  );
  ...
  const saveEmployee = async () => {
    ...
    setSaving(true);
    try {
      const { workDays, hoursPerDay } = scheduleToLegacy(
        editingEmployee.schedule,
      );
      ...
      if (editingEmployee.id) {
        ...
        const result = await supabase.from("home_employees").insert(data);
        error = result.error;
      }
  ...
  const getEmployeeHours = (emp: HomeEmployee): string => {
    if (emp.schedule) {
      const hours = calculateWeeklyHours(
        emp.schedule as Record<string, DaySchedule>,
      );
      return `${hours}h/sem`;
    }
                        ...
                        <p className="text-sm text-gray-500">
                          {emp.zone === "interior"
                            ...
                            : emp.zone === "exterior"
                              ...
                              : "🏡 Ambos"}
                          {" • "}
                          {emp.work_days?.length || 0} días •{" "}
                          {getEmployeeHours(emp)}
                        </p>
                      </div>
