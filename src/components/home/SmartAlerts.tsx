...
export default function SmartAlerts({
  ...
  const healthScore = Math.max(
    0,
    100 - highPriorityAlerts.length * 20 - otherAlerts.length * 5,
  );
                ...
                <>
                  {highPriorityAlerts.length > 0 && (
                    <span className="text-red-600 mr-2">
                      {highPriorityAlerts.length} alerta
                      {highPriorityAlerts.length > 1 ? "s" : ""} crítica
                      {highPriorityAlerts.length > 1 ? "s" : ""}
                    </span>
                  )}
                  {otherAlerts.length > 0 && (
                    <span className="text-yellow-600">
                      {otherAlerts.length} sugerencia
                ...
                {todayWorkload.map((workload) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, workload.utilizationPercent)}%`,
                          }}
                        />
                      </div>
                      <span
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {workload.assignedMinutes} / {workload.availableMinutes}{" "}
                      min
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-green-600">
                  La carga de trabajo está bien distribuida para los próximos 7
                  días
                </p>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-4 gap-2 text-xs">
                      <div className="text-center">
                        ...
                        <div className="font-medium">
                          {score.metrics.avgRating > 0
                            ? `${score.metrics.avgRating}/5`
                            : "-"}
                        </div>
                      </div>
                      <div className="text-center">
                    ...
                    {score.totalTasksCompleted > 0 ? (
                      <div className="text-xs text-gray-500 mt-2 text-center">
                        {score.totalTasksCompleted} tareas completadas •{" "}
                        {Math.round(score.totalMinutesWorked / 60)}h trabajadas
                      </div>
                    ) : score.isNewEmployee ? (
