...
export default function MonthlyReport({
  ...
  const loadReport = async () => {
    ...
    if (tasks) {
      ...
      const totalIssues =
        inspections?.reduce((sum, i) => sum + (i.issues_found || 0), 0) || 0;
      ...
      const topEmpId = Object.entries(empCompleted).sort(
        (a, b) => b[1] - a[1],
      )[0];
      const topEmp = topEmpId
        ? employees.find((e) => e.id === topEmpId[0])
        : null;
      ...
      const sortedSpaces = Object.entries(spaceCount).sort(
        (a, b) => b[1] - a[1],
      );
      const mostCleanedId = sortedSpaces[0];
      const leastCleanedId = sortedSpaces[sortedSpaces.length - 1];
      const mostCleaned = mostCleanedId
        ? spaces.find((s) => s.id === mostCleanedId[0])
        : null;
      const leastCleaned =
        leastCleanedId && sortedSpaces.length > 1
      ...
      setMonthStats({
        ...
        topEmployee: topEmp
          ? { name: topEmp.name, completed: topEmpId[1] }
          : undefined,
        mostCleanedSpace: mostCleaned
          ? {
              name:
                mostCleaned.custom_name || mostCleaned.space_type?.name || "",
              count: mostCleanedId[1],
            }
          : undefined,
        leastCleanedSpace: leastCleaned
          ? {
              name:
                leastCleaned.custom_name || leastCleaned.space_type?.name || "",
              count: leastCleanedId[1],
            }
          : undefined,
      });
      ...
      const empStats: EmployeeStats[] = employees.map((emp) => {
        ...
        const empCheckins =
          checkins?.filter((c) => c.employee_id === emp.id) || [];
        const empRatings =
          ratings?.filter((r) => r.employee_id === emp.id) || [];
        const empInspections =
          inspections?.filter((i) => i.employee_id === emp.id) || [];

        return {
          id: emp.id,
          name: emp.name,
          tasksCompleted: empTasks.length,
          hoursWorked: empCheckins.reduce(
          ...
          avgRating:
            empRatings.length > 0
              ? empRatings.reduce((sum, r) => sum + r.rating, 0) /
                empRatings.length
              : 0,
          issuesReported: empInspections.reduce(
            (sum, i) => sum + (i.issues_found || 0),
            0,
          ),
        };
      });
      setEmployeeStats(
        empStats.sort((a, b) => b.tasksCompleted - a.tasksCompleted),
      );

      // Space stats
      const sStats: SpaceStats[] = spaces.map((space) => {
        const spaceHistory =
          history?.filter((h) => h.space_id === space.id) || [];
        const spaceRatings =
          ratings?.filter((r) => r.space_id === space.id) || [];
        const lastClean = spaceHistory.sort(
          (a, b) =>
        ...
        return {
          ...
          avgRating:
            spaceRatings.length > 0
              ? spaceRatings.reduce((sum, r) => sum + r.rating, 0) /
                spaceRatings.length
              : 0,
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">
                          {monthStats.topEmployee.name}
                        </p>
                        <p className="text-sm text-blue-200">
                          {monthStats.topEmployee.completed} tareas completadas
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-500">
                        Espacio más atendido
                      </p>
                      <p className="font-medium">
                        {monthStats.mostCleanedSpace.name}
                      </p>
                      <p className="text-sm text-gray-400">
                        {monthStats.mostCleanedSpace.count} limpiezas
                      </p>
                    </div>
                  </div>
                )}
                ...
                {monthStats.leastCleanedSpace && (
                  <div className="bg-amber-50 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {monthStats.leastCleanedSpace.name}
                      </p>
                      <p className="text-sm text-amber-500">
                        {monthStats.leastCleanedSpace.count} limpiezas
                      </p>
                    </div>
                  </div>
                )}
                ...
                {monthStats.issuesFound > 0 && (
                  <div className="bg-red-50 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {monthStats.issuesFound} issues en inspecciones
                      </p>
                    </div>
                  </div>
                )}
