...
import { expandedRecipes, CATEGORY_CONFIG, ExpandedRecipe } from '@/data/expanded-recipes';
...
const CATEGORY_FILTERS: Array<{ key: 'all' | RecipeCategory; label: string; icon: string }> = [
...
export default function RecipesView({ recipes, onUpdate }: RecipesViewProps) {
  ...
  const filteredRecipes = useMemo(() => {
    return allRecipes.filter(recipe => {
      ...
      const matchesTag = recipe.tags?.some(
        tag => tag.toLowerCase().includes(searchLower)
      ) ?? false;
      const matchesSearch = !search || matchesName || matchesIngredient || matchesTag;
  ...
  const getRecipeBadges = (recipe: Recipe) => {
    ...
    if (recipe.thermomixCompatible) {
      badges.push({ label: '🤖 TM6', className: 'bg-purple-100 text-purple-700' });
    }
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto pb-2 scrollbar-hide" style={{ WebkitOverflowScrolling: 'touch' }}>
      ...
      <p className="text-xs text-gray-400 mb-3">
        ...
        {categoryFilter !== 'all' && ` en ${CATEGORY_FILTERS.find(c => c.key === categoryFilter)?.label}`}
      </p>
              ...
              <div className="flex">
                ...
                <div className="flex-1 p-3 flex justify-between items-start">
                  <div
                    ...
                    onClick={() => setSelectedRecipe(recipe)}
                  >
                    <h3 className="font-semibold mb-1 line-clamp-2 text-sm">{recipe.name}</h3>
                    <div className="flex flex-wrap gap-1 mb-1">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${getTypeClass(recipe.type)}`}>
